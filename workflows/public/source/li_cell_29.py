...
def hypervolume_min_botorch(Y_min, ref_min):
    """
    Y_min: (N,M) minimization PF points
    ref_min: (M,) reference point (worse than PF)
    returns: HV (float)
    """
    Y = torch.as_tensor(Y_min, dtype=torch.double)
    ref = torch.as_tensor(ref_min, dtype=torch.double)

    # BoTorch Hypervolume 默认是 “maximize”
    # 把 minimization 转成 maximization：Y_max = -Y_min，ref_max = -ref_min
...
def multi_seed_pf_compare(
    ...
    *,
    ...
    records = []
    pf_store = {"raw": [], "raw_uncertainty": [], "cal": []}
    ...
    for seed in seeds:
        ...
        hv_raw = hypervolume_min_botorch(Y_raw, ref)
        ...
        # ---------- PF: raw uncertainty ----------
        X_raw_unc, Y_raw_unc = run_pf_once(auto, use_calibration=False, transform="ucb", beta=1., pop_size=pop_size, n_gen=n_gen, seed=seed)
        pf_store["raw_uncertainty"].append((seed, X_raw_unc, Y_raw_unc))

        hv_raw_unc = hypervolume_min_botorch(Y_raw_unc, ref)
        ...
        pf_store["cal"].append((seed, X_cal, Y_cal))

        hv_cal = hypervolume_min_botorch(Y_cal, ref)
